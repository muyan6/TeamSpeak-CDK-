...
1. **智能端口分配与冲突规避**：
   - **基础端口避让**：规避已被宿主机占用的 TS3 官方基础端口（`9987/udp`、`30033/tcp`、`10011/tcp`、`41144/tcp/udp`）。
   - **自增规则**：
     - `ts1`：`9988` (Voice), `30034` (File), `10012` (Query), `41145` (TSDNS)
     - `ts2`：`9989` (Voice), `30035` (File), `10013` (Query), `41146` (TSDNS)
     - `tsN`：`9987 + N`, `30033 + N`, `10011 + N`, `41144 + N`
   - **双重防冲突**：数据库查重 + 操作系统 Socket 实时探测，被占用则自动跳过。
